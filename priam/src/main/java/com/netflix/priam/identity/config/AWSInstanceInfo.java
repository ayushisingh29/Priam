...
public class AWSInstanceInfo implements InstanceInfo {
            ...
            try {
                ...
                String vpcurl =
                        "http://169.254.169.254/latest/meta-data/network/interfaces/macs/"
                                + nacId
                                + "vpc-id";
                logger.info("ayushis: vpc url :{}", vpcurl);
                String oldvpcId =
                        SystemUtils.getDataFromUrl(
                                        "http://169.254.169.254/latest/meta-data/network/interfaces/macs/"
                                                + nacId
    ...
    public String getHostname() {
        if (hostName == null) {
            ...
            hostName =
                    newTryGetDataFromUrl(NEW_PUBLIC_HOSTNAME_URL)
                            .orElse(EC2MetadataUtils.getData(NEW_LOCAL_HOSTNAME_URL));
    ...
    public String getHostIP() {
        if (hostIP == null) {
            ...
            hostIP =
                    newTryGetDataFromUrl(NEW_PUBLIC_HOSTIP_URL)
                            .orElse(EC2MetadataUtils.getData(NEW_LOCAL_HOSTIP_URL));
